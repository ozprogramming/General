...
Matrix* matrixScalarOperation(Matrix* matrix, double scalar, char operation)
{
    Matrix* operated = newMatrix(matrix->rows, matrix->cols, 0.0);
    ...
    for (int i = 0; i < matrix->rows; ++i)
    {
        for (int j = 0; j < matrix->cols; ++j)
        {
            ...
            {
                case '^':
                    operated->entries[i][j] = pow(matrix->entries[i][j], scalar);
                    break;
                case '*':
                    operated->entries[i][j] = matrix->entries[i][j] * scalar;
                    break;
                case '/':
                    operated->entries[i][j] = matrix->entries[i][j] / scalar;
                    break;
...
{
    Matrix* diff = newMatrix(A->rows, A->cols, 0);

    for (int i = 0; i < A->rows; i++)
    {
        for (int j = 0; j < A->cols; j++)
        {
            diff->entries[i][j] = A->entries[i][j] - B->entries[i][j];
        }
    }
...
Matrix* hadamardProduct(Matrix* A, Matrix* B)
{
    ...
    for (int i = 0; i < A->rows; i++)
    {
        for (int j = 0; j < A->cols; j++)
        {
            product->entries[i][j] = A->entries[i][j] * B->entries[i][j];
        }
    }
